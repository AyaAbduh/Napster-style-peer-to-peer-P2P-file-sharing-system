...
def DownloadasServer():
  ...
  s.bind((host, peer2Port))  # Bind to the port
...
#constants
peer2Port=54492
filelist=['Artificial.txt','robotics.txt','mytext.txt']

peer2=Client(peer2Port)  #object from client
  ...
  if choice=='1':
    ...
    while True:
      ...
      if not filename:
        ...
        PORTLIST=peer2.findFile(filename)
        if not PORTLIST:
