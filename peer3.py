...
def DownloadasServer():
  ...
  s.bind((host, peer3Port))  # Bind to the port
...
#constants
peer3Port=54493
filelist=['Artificial.txt','heading.txt','mytext.txt']

peer3=Client(peer3Port)  #object from client
  ...
  if choice=='1':
    ...
    while True:
      ...
      if not filename:
        ...
        peerlist=peer3.findFile(filename)
        break
  elif choice=='3':
    while True:
            Flag=False
            filename = raw_input("Enter file Name ")
            PORTLIST = peer3.findFile(filename)
            # print PORTLIST
            if not PORTLIST:
                print "File doesn't exist in the network"
                pass
            else:
               #serverPeer = '4567'  # peer1
